...
public: void aMethod() {//method deffintion //a method is a fiunction inside a class and can access variables inside the class

    cout << "\n" << "Hello from a method!!" << endl; //The method displays a sentence
}
};

// methods can also be defined outside the class the following way: // inside the class: declare the function and its inputs // outside the class: (function type) className :: mthodName() // the :: is used to define the scope of the function

//Class called coffee with a method that takes parameters //and a method definition outside the class
...
//Access specifier define how the attributes and methods of the class can be access //Public: members are accessible from outside the class //Private: members cant be accessed from outside (only through methods) //Protected: members can be accessed only by inherated classes
...
//Encapsulation protects data from users by using privates attributes //encapsulation uses getters and setters //setters/mutators can defined and interact with the private attrs //getters display the information
...
//We can pass attributes and methods from one class to another //Inheritance allow us to reuse attributes and methods //Child/Derived class: the class that is borrowing from other classes //Parent/Base class: The class that is borrowed from
...
//drinks is broad enough to be a parent //there can be different drink types: wines, beers, and coctails //let say we want to make a derived class from drinks just for beers //all the drived classes will use the name, price and the default constructor from drinks
...
//we can have a multi-level inheretance; Cider is the granchild of Drinks and Child of beers //A more ideal example would be having the following classes: Beer derived from alcoho, and alcohol be derived from Drinks
...
//We can also do multi-inheratence //instead of having one base class we can have two and have a combination of all of methods and attributes //lets create a new class named alcohol
...
//Child classes can access private attributes of other classes //This is called the protected access specifier
...
class DepartmentInfo: public Company{ public:
...
//Polymorphism means "many forms" and the concept is based around the use of inheretance to perform differen taks //The goal of polymorphism is to reuse code //lets make a new example
